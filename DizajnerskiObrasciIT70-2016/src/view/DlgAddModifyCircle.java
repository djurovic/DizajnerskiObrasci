...
public class DlgAddModifyCircle extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public DlgAddModifyCircle(MainFrame mf, Circle c) {
		this.circle = c;
		this.mf = mf;
		...
		{
			{
				...
				if (circle != null) {
					txtRadius.setText(String.valueOf(circle.getR()));
				}
		...
		if (circle != null) {
			{
				...
				txtColorBorder.setBackground(c.getColor());
		...
		if (circle != null) {
			...
			txtColorFill.setBackground(c.getColorFill());
					...
					public void actionPerformed(ActionEvent e) {
						if (circle != null) {
							try {
								...
								Color colorBorder = txtColorBorder.getBackground();
								Color colorFill = txtColorFill.getBackground();
								
								Circle edited = new Circle(new Point(x, y), radius, colorFill, colorBorder);
								edited.setName(circle.getName());
								edited.setSelected(true);
								
								EditCommand cmd = new EditCommand(circle, edited, mf.getCanvas().getShapes());
...
//								mf.logCommand(cmd.toString());
							...
							try{
								...
								Shape s = new Circle(mf.getClickedLocation(), radius, mf.getBtnFillColor().getBackground(), mf.getBtnBorderColor().getBackground());
